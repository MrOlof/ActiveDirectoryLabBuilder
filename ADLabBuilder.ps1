<#
.SYNOPSIS
Builds out an Active Directory lab environment with AD DS install, forest creation,
OUs and accounts.

.DESCRIPTION
This script can:
 * Install the AD DS role on a fresh Windows Server
 * Create a new AD forest and promote the local server to a domain controller
 * Build out an AD lab structure with OUs, users, groups, service accounts, gMSAs,
   computers, FGPPs, delegation scenarios, etc.

It supports both parameter-based execution and an interactive mode (yes/no prompts)
when no feature switches are provided.

.PARAMETER Domain
Domain Name of your AD (also used as the forest root domain when creating a new forest)

.EXAMPLE
PS> .\Invoke-ADLabBuildOut.ps1

Preferred method – runs in interactive mode.

.EXAMPLE
PS> .\Invoke-ADLabBuildOut.ps1 -Domain activedirectory.lab -InstallADForest -CreateTopLevelOUs -CreateADLabUsers

Installs AD DS, creates a new forest activedirectory.lab (reboots),
then you re-run the script to build out the lab (OUs, users etc).
 ...
 * Windows Server 2016 or later (for AD DS forest creation)
 * Active Directory PowerShell Module
 * Group Policy PowerShell Module
 * Run as elevated Administrator
#>

# Script Version 1.26.11.21 (AD DS + forest creation support added)

Param
(
    [string]$Domain,
    [switch]$InstallADForest,
...
# Ensure we are running elevated (required for AD DS install / forest creation)
if (-not ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole(
    [Security.Principal.WindowsBuiltInRole]::Administrator
)) {
    Write-Error "This script must be run as an elevated Administrator."
    return
}
...
Function Create-ADLabUsers
{
    ...
    {  
        ...
        SWITCH ($NameFormatLayout)
        {
            ...
            Default             { $TestUserAccount = $UserFirstName + '.' + $UserLastName }
        }
    ...
    While ($UserAccountLoopCount -lt $NumberOfADUserAccounts)
...
Function Create-ADLabServiceAccounts
{
    ...
    { 
        ...
        IF ($LabServiceAccountArrayItem -eq 'Acronis')   { Invoke-Expression $AcronisSPN  }
        IF ($LabServiceAccountArrayItem -eq 'AGPM')      { Invoke-Expression $AGPMSPN    }
        IF ($LabServiceAccountArrayItem -eq 'BigFix')    { Invoke-Expression $AGPMSPN    }
        IF ($LabServiceAccountArrayItem -eq 'CAXOER')    { Invoke-Expression $CAXOSPN    }
        IF ($LabServiceAccountArrayItem -eq 'CheckPoint'){ Invoke-Expression $CHECKPOINTSPN }
        IF ($LabServiceAccountArrayItem -eq 'Citrix')    { Invoke-Expression $CITRIXSPN  }
        IF ($LabServiceAccountArrayItem -eq 'CitrixPVS') { Invoke-Expression $CitrixPVSSPN }
        IF ($LabServiceAccountArrayItem -eq 'Cloudera')  { Invoke-Expression $CLOUDERASPN }
        IF ($LabServiceAccountArrayItem -eq 'Cognos')    { Invoke-Expression $COGNOSSPN  }
        IF ($LabServiceAccountArrayItem -eq 'Dynamics')  { Invoke-Expression $DYNAMICSSPN }
        IF ($LabServiceAccountArrayItem -eq 'Exchange')  { Invoke-Expression $EXCHSPN    }
        IF ($LabServiceAccountArrayItem -eq 'FIM')       { Invoke-Expression $FIMSPN     }
        IF ($LabServiceAccountArrayItem -eq 'Flume')     { Invoke-Expression $FLUMESPN   }
        IF ($LabServiceAccountArrayItem -eq 'Hadoop')    { Invoke-Expression $HADOOPSPN  }
        IF ($LabServiceAccountArrayItem -eq 'Impala')    { Invoke-Expression $IMPALASPN  }
        IF ($LabServiceAccountArrayItem -eq 'InfoSphere'){ Invoke-Expression $InfoSphereSPN }
        IF ($LabServiceAccountArrayItem -eq 'JBoss')     { Invoke-Expression $JBOSSSPN   }
        IF ($LabServiceAccountArrayItem -eq 'Kafka')     { Invoke-Expression $KAFKASPN   }
        IF ($LabServiceAccountArrayItem -eq 'MagFS')     { Invoke-Expression $MagFSSPN   }
        IF ($LabServiceAccountArrayItem -eq 'Mongo')     { Invoke-Expression $MongoSPN   }
        IF ($LabServiceAccountArrayItem -eq 'SQL')       { Invoke-Expression $MSSQLSPN   }
        IF ($LabServiceAccountArrayItem -eq 'OpenAccess'){ Invoke-Expression $OpenAccessSPN }
        IF ($LabServiceAccountArrayItem -eq 'Oracle')    { Invoke-Expression $OracleSPN  }
        IF ($LabServiceAccountArrayItem -eq 'Quest')     { Invoke-Expression $QuestSPN   }
        IF ($LabServiceAccountArrayItem -eq 'SAPBO')     { Invoke-Expression $SAPSPN     }
        IF ($LabServiceAccountArrayItem -eq 'SCOM')      { Invoke-Expression $SCOMSPN    }
        IF ($LabServiceAccountArrayItem -eq 'VMWare')    { Invoke-Expression $VMwareSPN  }
        IF ($LabServiceAccountArrayItem -eq 'Web')       { Invoke-Expression $WWWSPN     }

    }
    WHILE ($SerivceAccountLoopCount -lt $NumberOfAccounts) 
...
Function Create-ADLabAdminAccounts
{
    ...
    (
        [Parameter(Mandatory = $true)]$Domain,        
        [Parameter(Mandatory = $true)][int]$NumberOfAdminAccounts, # actual # may be slightly less due to collisions
    ...
    While ($AdminAccountLoopCount -lt $NumberOfAdminAccounts)
...
Function Create-ADLabGMSAs
{
    ...
    ELSE
    {
        ...
        { 
            ...
            $DomainDC2 = $DC2 -Replace (',DC=',".")
            $KDSDomainArray = Get-ADDomain -Server $DomainDC2
        ...
        $KDSDomainCheck = $KDSRootKeyDomainArray | Where-Object {$_.DNSRoot -eq $Domain}
    ...
    WHILE ($GMSADoWhileLoop -lt $NumberofGMSAs)
...
Function Create-ADLabWindowsWorkstations
{
    ...
    $WorkstationOperatingSystemArray = @(
        'Windows XP Professional','Windows 7 Enterprise','Windows 7 Professional','Windows 7 Ultimate',
    ...
    DO
    {
        ...
        $ComputerAlpha           = -join ((65..90) | Get-Random -Count 3 | ForEach-Object {[char]$_})
    ...
    WHILE ($DoWhileWorkstationLoop -lt $NumberOfWorkstations)
}
...
Function Create-ADLabWindowsServers
{
    ...
    $WindowsServerOperatingSystemArray = @(
        ...
        'Windows Server 2022 Standard','Windows Server 2022 Enterprise',
        'Windows Server 2025 Standard','Windows Server 2025 Enterprise'
    )
    ...
    WHILE ($DoWhileServerLoop -lt $NumberOfservers)
}
...
Function Create-ADLabComputers
{
    ...
    {
        $DoWhileServerLoop++
        Write-Host "Creating Non-Windows computer account $DoWhileServerLoop of $NumberOfComputers"
    ...
    WHILE ($DoWhileServerLoop -lt $NumberOfComputers)
}
...
Function Create-ADLabFGPPs
{
    ...
    {
        $FGPPDOWhileLoop++
        Write-Host "Create Fine-Grained Password Policies in $($DomainInfo.Name) ($FGPPDOWhileLoop of $NumberOfFGPPs)..." -ForegroundColor Cyan
    ...
    WHILE ($FGPPDOWhileLoop -lt $NumberOfFGPPs) 
}
...
Function Set-SPNDefaultAdminAccount
{
    ...
    CATCH
    { Write-Warning "Unable to set the SPN $SPN on the default Administrator account using the DC $DomainDC" }  
}
...
Function Add-KerberosDelegation
{
    ...
    DO
    {
        ...
        { $TrustedForDelegationBoolean = $True } # Enables Unconstrained Delegation 
...
Function Add-ComputerAccountstoAdmins
{
    ...
    WHILE ($DoWhileAddComputerLoop -lt $NumberOfComputerAccountsInAdmin) 
}
...
Function Add-HybridServiceAccounts
{
    ...
    $PasswordNeverExpiresValue = $True
...
Function Set-OUsWithBlockedGPOInheritance
{
    ...
    WHILE ($DOWhileOULoop -le $NumberOfOUs)     
}

################################################
# NEW: AD DS INSTALL + FOREST CREATION
################################################

Function Install-ADDSForestInteractive {
    Param(
        [Parameter(Mandatory = $true)][string]$DomainName
    )

    # Check if AD DS role / forest creation is even possible
    $adDsFeature = Get-WindowsFeature -Name AD-Domain-Services -ErrorAction SilentlyContinue
    if (-not $adDsFeature) {
        Write-Error "Get-WindowsFeature / AD-Domain-Services not available. This must run on Windows Server (with ServerManager module)."
        return
    }

    # Check if this server already looks like a DC
    $ntdsService = Get-Service -Name NTDS -ErrorAction SilentlyContinue
    if ($ntdsService) {
        Write-Host "This server already appears to be a domain controller. Skipping forest creation."
        return
    }

    if (-not $adDsFeature.Installed) {
        Write-Host "Installing AD DS role (AD-Domain-Services) and management tools..."
    ...
    Write-Host ""
    Write-Host "About to create a NEW forest with root domain '$DomainName' on THIS server." -ForegroundColor Cyan
    Write-Host "The server WILL reboot as part of promotion." -ForegroundColor Yellow
    Write-Host "After reboot, log on as the domain Administrator and re-run this script to build the lab OUs & accounts." -ForegroundColor Yellow
    Write-Host ""

    $confirm = Read-Host "Type 'YES' to continue with forest creation (anything else cancels)"
    if ($confirm -ne 'YES') {
        Write-Host "Forest creation cancelled."
        return
    }

    $SafeModePassword = Read-Host "Enter DSRM (Directory Services Restore Mode) password" -AsSecureString
...
if (-not $Domain) {
    $Domain = Read-Host "Enter the AD domain name (forest root, e.g. lab.local)"
}

# Detect if this server already is a DC
...
# Detect if any feature switches were explicitly passed
$featureSwitches = @(
    'InstallADForest',
    'CreateTopLevelOUs','CreateBranchOfficeOUs','RenameDomainAdministrator',
    'CreateADLabUsers','CreateADLabGroups','CreateADLabServiceAccounts','CreateADLabAdminAccounts',
    'CreateADLabGMSAs','CreateADLabWindowsWorkstations','CreateADLabWindowsServers','CreateADLabComputers',
    'CreateADLabFGPPs','SetSPNDefaultAdminAccount','InvokeRandomizeAdmins','InvokeRandomizeServiceAccountAdmins',
    'AddPasswordToADAttribute','AddKerberosDelegation','AddComputerAccountstoAdmins','SetOUsWithBlockedGPOInheritance'
)

$anySwitchSet = $false
foreach ($fs in $featureSwitches) {
...
# If no switches provided, go interactive
if (-not $anySwitchSet) {
    Write-Host "==========================================" -ForegroundColor Cyan
    Write-Host "     AD Lab Build-Out – Interactive Mode  " -ForegroundColor Cyan
    Write-Host "Domain / Forest root: $Domain" -ForegroundColor Cyan
    Write-Host "==========================================" -ForegroundColor Cyan

    if (-not $IsDomainController) {
        $InstallADForest = Read-YesNo "This server is not a domain controller. Install AD DS role and create a new forest '$Domain' on this machine? (Requires reboot)" 'N'
    }

    $CreateTopLevelOUs              = Read-YesNo "Create top-level OU structure (AD Administration, Enterprise Services, Domain Users, Workstations)?" 'Y'
    $CreateBranchOfficeOUs          = Read-YesNo "Create branch office OUs (city-based structure)?" 'N'
    $RenameDomainAdministrator      = Read-YesNo "Rename the default 'Administrator' account to 'TRDAdministrator'?" 'N'
    $CreateADLabUsers               = Read-YesNo "Create 20 lab user accounts in 'OU=Domain Users'?" 'Y'
    $CreateADLabGroups              = Read-YesNo "Create basic lab groups (Workstation Admins, Server Admins)?" 'Y'
    $CreateADLabServiceAccounts     = Read-YesNo "Create 10 lab service accounts under 'OU=Service Accounts,OU=Enterprise Services'?" 'Y'
    $CreateADLabAdminAccounts       = Read-YesNo "Create 10 lab admin accounts in 'OU=Accounts,OU=AD Administration'?" 'Y'
    $CreateADLabGMSAs               = Read-YesNo "Create 10 gMSA accounts (and KDS root key if missing)?" 'N'
    $CreateADLabWindowsWorkstations = Read-YesNo "Create 20 Windows workstation computer accounts in 'OU=Workstations'?" 'Y'
    $CreateADLabWindowsServers      = Read-YesNo "Create 20 Windows Server computer accounts in 'OU=Servers,OU=Enterprise Services'?" 'Y'
    $CreateADLabComputers           = Read-YesNo "Create 20 non-Windows computer accounts in 'OU=Servers,OU=Enterprise Services'?" 'N'
    $CreateADLabFGPPs               = Read-YesNo "Create 10 Fine-Grained Password Policies (FGPPs)?" 'N'
    $SetSPNDefaultAdminAccount      = Read-YesNo "Add a random MSSQL SPN to the default Administrator account?" 'N'
    $InvokeRandomizeAdmins          = Read-YesNo "Randomly add admin users into privileged groups?" 'N'
    $InvokeRandomizeServiceAccountAdmins = Read-YesNo "Randomly add service accounts into privileged groups?" 'N'
    $AddPasswordToADAttribute       = Read-YesNo "Randomly write a password-like string into a service account's 'info' attribute?" 'N'
    $AddKerberosDelegation          = Read-YesNo "Randomly create Kerberos delegation scenarios (unconstrained / constrained)?" 'N'
    $AddComputerAccountstoAdmins    = Read-YesNo "Randomly add computer accounts as members of privileged groups?" 'N'
    $SetOUsWithBlockedGPOInheritance = Read-YesNo "Randomly set Blocked GPO Inheritance on some OUs under 'OU=Branch Offices'?" 'N'
}

################################################
# MAIN EXECUTION (COMMON FOR PARAM + INTERACTIVE)
################################################

# 1) If requested, install AD DS and create forest first, then bail (server will reboot)
if ($InstallADForest -eq $True) {
    Install-ADDSForestInteractive -DomainName $Domain
    return
}

# From here on we assume we are running ON a domain controller in the new/existing forest.
...
{
    Create-ADLabUsers -Domain $Domain -NumberOfADUserAccounts '20' -FirstNameFile 'C:\Scripts\FirstNames.csv' -LastNameFile 'C:\Scripts\LastNames.csv' -UserOU "OU=Domain Users" -NameFormatLayout 'FirstNameLastName' -EnableAccounts
}
...
{
    Create-ADLabAdminAccounts -Domain $Domain -AdminNamePrefix 'Admin' -NumberOfAdminAccounts '10' -FirstNameFile 'C:\Scripts\FirstNames.csv' -LastNameFile 'C:\Scripts\LastNames.csv' -AccountOU 'OU=Accounts,OU=AD Administration' -NameFormatLayout 'FirstL'
}
...
IF ($CreateADLabWindowsWorkstations -eq $True)
{
